...
def index():
    if request.method == 'POST':
        try:
            ...
            return render_template('index.html', message=f"Error al procesar los juegos: {str(e)}")
        ...
        try:
            ...
            for game, details in tr_games_info.items():
                if details:  # Check if details list is not empty
                    ...
                    data.append({'Name': 'Not found', 'Platform': 'N/A', 'Price (TRY)': 'N/A', 'Price (USD)': 'N/A'})

            if not data:
                return render_template('index.html', message="No se han encontrado resultados para los juegos buscados.")
        except Exception as e:
            return render_template('index.html', message=f"Error al compilar los datos de los juegos: {str(e)}")
        ...
        try:
            ...
            return render_template('index.html', message=f"Error al obtener la tasa de cambio: {str(e)}")
        ...
        try:
            ...
            return render_template('index.html', message=f"Error al limpiar y convertir columnas de precios: Columna no encontrada {str(e)}")
        except Exception as e:
            return render_template('index.html', message=f"Error al limpiar y convertir columnas de precios: {str(e)}")
...
def fetch_game_info(urls, items, region):
    ...
    for url, item in zip(urls, items):
        try:
            ...
            for product in products:
                ...
                if name_tag and platform_tag:
                    ...
                    if price_discount_tag:
                        price = price_discount_tag.get_text(strip=True)
                    elif price_tag:
            ...
            print(f"Error general procesando {item} ({region}): {str(e)}")
            if item not in games_info:
                games_info[item] = [{
                    'Name': 'No encontrado',
                    f'Price ({region})': 'N/A',
